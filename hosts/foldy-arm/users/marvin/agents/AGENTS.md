...
Agent runs natively on macOS (aarch64-darwin) with nix-darwin.
...
- **Core / coreutils**: `ls`, `cat`, `head`, `tail`, `cp`, `mv`, `rm`, `mkdir`,
  `ln`, `chmod`, `chown`, `stat`, `find`, `tree`, `du`, `df`, `wc`, `sort`,
  `uniq`, `cut`, `tr`, `tee`, `xargs`, `env`, `printenv`, `which`, `file`
- **Text processing**: `grep`, `sed`, `awk`, `diff`, `cmp`, `less`, `jq`, `fzf`,
  `rg`
...
Notable tools **not** on PATH (use `nix run nixpkgs#<name>`): `gh`, `fd`, `bat`,
`sops`, `age`, `nvim`, `opentofu`, `node`, `npm`. Note: only `python3` exists,
not `python`.

**Permanent:** add the tool to the nix-darwin config in the dotfiles repo at
